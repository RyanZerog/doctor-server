...
import { Row, Col, Card, Statistic, List, Tag, Typography, Alert, Button, Progress, Divider } from 'antd';
import { 
  TeamOutlined, 
  CalendarOutlined, 
  BellOutlined, 
        ...
        <Col xs={24} sm={12} md={6}>
          <Card className="stat-card recovery-card">
            <Statistic
              ...
              suffix="%"
              precision={0}
              prefix={<MedicineBoxOutlined className="stat-icon" />}
            ...
            {sortedCauseStats.length > 0 ? (
              <div className="cause-stats">
                {sortedCauseStats.map((item, index) => (
                  <div className="cause-stat-item" key={item.cause}>
